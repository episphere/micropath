...
const OPENJPEG_LIB_URL = {
  "js": "https://cdn.jsdelivr.net/npm/@cornerstonejs/codec-openjpeg@1.2.2/dist/openjpegjs.js",
  "wasm": "https://cdn.jsdelivr.net/npm/@cornerstonejs/codec-openjpeg@1.2.2/dist/openjpegwasm.js"
}
  
var imagebox3 = (function() {
    ...
    self.addEventListener("fetch", (e) => {
      ...
      if (e.request.url.startsWith(self.tileServerBasePath)) {
        ...
        if (e.request.url.endsWith("/info.json")) {
          ...
          e.respondWith(imagebox3.getImageTile(decodeURIComponent(identifier), tileParams), true)
        }        
      }
...
(function ($){
  ...
  const getImageInfo = async (imageID, cache=true) => {
  ...
  const getImagesInPyramid = async (imageID, cache=true) => {
    ...
    let cacheHeaders = undefined
    if (!cache) {
      cacheHeaders = {'Cache-Control': "no-cache, no-store"}
    }

    try {
     
      tiff[imageID].pyramid = tiff[imageID].pyramid || ( await GeoTIFF.fromUrl(imageID, { headers: cacheHeaders }) )
  ...
  const getImageThumbnail = async (imageID, tileParams, pool=false, cache=true) => {
    ...
    if (pool && !$.ENVIRONMENT_IS_SERVICE_WORKER) {
      await createPool(thumbnailImage?.fileDirectory.Compression)
    }
  ...
  const getImageTile = async (imageID, tileParams, pool=false, cache=true) => {
    ...
    if (!(tiff[imageID] && tiff[imageID].pyramid) || tiff[imageID].pyramid.loadedCount === 0) {
      await getImagesInPyramid(imageID, false, cache)
    }
    ...
    if (pool && !$.ENVIRONMENT_IS_SERVICE_WORKER) {
      await createPool(optimalImageInTiff.fileDirectory?.Compression)
    }
  ...
  const createPool = async (compressionMethod=7) => {
    // if (!$.workerPool || $.workerPool.compressionSupported !== compressionMethod) {
    //   if (compressionMethod === 33005) {
        
    //   document.body.appendChild(openJPEGScript)
    //       }
    //     }
    //   } else {
    //     $.workerPool = new Pool(Math.floor(navigator.hardwareConcurrency/2))
    //   }
    //   $.workerPool[compressionSupported] = compressionMethod
